 ...
 * Example usage:
 * ```
 *     return res.myRedirect();
 *     // -or-
 *     return res.myRedirect(optionalData);
 * ```
 *
 * Or with actions2:
 * ```
...
module.exports = function myRedirect(optionalData) {
  // Get access to `req` and `res`
  var req = this.req
  var res = this.res

  // Define the status code to send in the response.
  var statusCodeToSet = 303

  // If no data was provided, use res.sendStatus().
  if (optionalData === undefined) {
    sails.log.info('Ran custom response: res.myRedirect()')
  ...
  // except in production use res.sendStatus().
    ...
    sails.log.info(
      'Custom response `res.myRedirect()` called with an Error:',
      optionalData
    )
  ...
  // Set status code and send response data.
    ...
    return res.redirect(statusCodeToSet, optionalData)
  }
}
